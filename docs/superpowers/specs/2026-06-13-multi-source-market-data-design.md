...
ne les a pas, **aussi bien que `../portfodor/`** — en particulier `LU0131510165` et `LU1832174962`.
...
- **Financial Times = la source des fonds FR/LU**, et **vérifié joignable d'ici** :
  ...
  - **Live OK** : LU1832174962 → close 241,79 € (2026-06-11, xid 118135654) ; LU0131510165 → 940,74 € (xid 8542). Devise EUR.
...
- **Morningstar via Boursorama** (fallback défensif, **non vérifiable d'ici** — l'endpoint NAV renvoie `[]`) :
  ...
  - Morningstar : `GET https://tools.morningstar.fr/api/rest.svc/timeseries_price/ok91jeenoo?id={0P…}&idtype=Morningstar&frequency=daily&startDate={YYYY-MM-DD}&outputType=COMPACTJSON` →
    `[[epoch_ms, value], …]`.
- **Eres `990000118919`** : code interne AMF (FCPE/PEE), pas un ISIN coté — **couvert par aucun provider** (FT/Boursorama renvoient vide), portfodor non plus. → reste en `asset set` manuel (documenté). Une source France-spécifique (AMF GECO) serait un chantier séparé, hors scope.
...
- **Hors scope (documenté)** : FCPE/PEE par code AMF (Eres) → `asset set` manuel ; Stooq (ticker-only,
...
3. **`Multi` + câblage** : chaîne Yahoo→FT, `market.Default()`, `cli` wire `Default()` au lieu de `NewYahoo()`. Active le fallback FT. **Vérif live LU0131510165/LU1832174962.**
4. **Morningstar/Boursorama** : provider défensif + tests.
5. **Doc** : README (« atypical assets / funds by ISIN »), note sur l'Eres FCPE manuel, DECISIONS.

## 5. Critères de réussite

1. `finador refresh` cote **LU1832174962** et **LU0131510165** via FT (séries NAV EUR) — **vérifié live**.
...
4. Eres FCPE : documenté comme `asset set` manuel.
