...
// Name is free-form ("PEA BforBank"); ID is its stable slug.
type Account struct {
...
func TestResolveAccount(t *testing.T) {
	b := sampleBook(t)
	for _, ref := range []string{"pea-bforbank", "PEA BforBank", "pea bforbank"} {
		if _, err := b.Account(ref); (ref == "pea bforbank") == (err == nil) {
			// "pea bforbank" ne matche ni l'ID ni le nom exact → ErrNotFound
...
func TestAccountAddAndList(t *testing.T) {
	...
	for _, want := range []string{"pea-bforbank", "PEA BforBank", "gains:17.2%", "cto-saxo", "USD"} {
