...
**Goal:** La v0.2 demandée par l'utilisateur : références courtes non ambiguës (`add cw8`), exclusions d'actifs dans toute portée (`perf --exclude btc,ddog`), ventilation par enveloppe (`value --by enveloppe`, aussi au web), hypothèses jetables (`value --what-if ddog=280` avec delta), `asset edit/rm` (gestion des alias, ISIN, retenue à la source sur dividendes), couleurs ANSI dans `perf`, et verrouillage optimiste inter-processus (CLI ↔ serve).
...
- **What-if** : prix exprimés dans la devise de cotation de l'actif (`ddog=280` = 280 USD si DDOG cote en USD), jamais persistés, uniquement sur `value`. L'affichage donne la valeur hypothétique ET le delta vs réel.
...
préfixe « ddog »). Dans le même task, basculer AddAccount et AddAsset sur une
...
existant « PEA BforBank » au lieu d'en créer un second : comportement VOULU (moins de
...
- [ ] **Step 4: vérifier** - `go test ./... -count=1` TOUT vert (les suites existantes
  ...
  ATTENTION à `TestResolveAccount` existant : « bforbank » n'est PAS un préfixe (c'est
