...
const boursoSearchOK = `<div class="search__results">
  <a href="/bourse/opcvm/cours/0P0001ABCD/" class="search__item-link">
    <span class="search__item-title">Convex AM Europe Small</span>
  </a>
</div>`
...
func TestMorningstarDailyOK(t *testing.T) {
	...
	got, err := ms.Daily(context.Background(), Ref{ISIN: "LU1111111111"}, mustDate("2023-11-14"))
	if err != nil {
		t.Fatal(err)
	}
	if len(got.Closes) != 2 {
...
func TestMorningstarEmptyResult(t *testing.T) {
	...
	_, err := ms.Daily(context.Background(), Ref{ISIN: "LU1111111111"}, mustDate("2023-11-10"))
	if !errors.Is(err, ErrNotCovered) {
