...
func TestScopeViews(t *testing.T) {
	...
	for path, want := range map[string][]string{
		...
		"/account/pea":         {"PEA BforBank", "cash", "recent transactions"},
...
func TestImportUpload(t *testing.T) {
	...
	part.Write([]byte("date,kind,account,asset,quantity,price,amount,currency,group,note\n" +
		"2026-02-01,buy,PEA BforBank,CW8.PA,3,540,,EUR,actions/monde,import web\n"))
	mw.Close()
...
func TestTxListAndAdd(t *testing.T) {
	...
	for _, want := range []string{"PEA BforBank", "Amundi MSCI World", "<form", `name="kind"`} {
		if !strings.Contains(body, want) {
...
func TestAssetsCSV(t *testing.T) {
	...
	if !strings.Contains(body, "cash,,PEA BforBank,") {
