...
func TestAccountsListAndCreate(t *testing.T) {
	...
	for _, want := range []string{"PEA BforBank", "gains:17.2%", "<form", `name="tax-mode"`, `href="/accounts"`} {
	...
	code, body, loc := postForm(t, srv, "/accounts", url.Values{
		"name": {"CTO Saxo"}, "ccy": {"USD"},
		"tax-mode": {"gains"}, "tax-rate": {"30"}, "aliases": {"cto, saxo"},
	})
	...
	if acc.Name != "CTO Saxo" || acc.Currency != "USD" || acc.Tax.String() != "gains:30%" {
		t.Errorf("created account = %+v, tax %s", acc, acc.Tax)
	}
	if len(acc.Aliases) != 2 || acc.Aliases[0] != "cto" || acc.Aliases[1] != "saxo" {
		t.Errorf("aliases = %v, want [cto saxo]", acc.Aliases)
	}
	...
	// duplicate name → 400.
	code, _, _ = postForm(t, srv, "/accounts", url.Values{"name": {"PEA BforBank"}, "ccy": {"EUR"}})
	if code != 400 {
...
func TestAccountEdit(t *testing.T) {
	...
	for _, want := range []string{`value="PEA BforBank"`, `value="gains" selected`, `value="17.2"`} {
		if !strings.Contains(body, want) {
