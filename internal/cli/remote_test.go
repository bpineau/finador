...
func TestRemoteWritePushesUpdatedContent(t *testing.T) {
	...
	mustRunRemote(t, fake, "account", "add", "CTO Saxo")
	...
	out := mustRunRemote(t, fake, "account", "list")
	if !strings.Contains(out, "CTO Saxo") {
...
func TestRemoteCompactPushes(t *testing.T) {
	...
	if out := mustRunRemote(t, fake, "account", "list"); !strings.Contains(out, "PEA BforBank") || !strings.Contains(out, "CTO Saxo") {
