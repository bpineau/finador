...
func perfCmd(a *app) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  finador perf\n" +
			...
			"  finador perf --tree            # per-envelope tree: gross, net, 1d/3d/7d/1m/3m/ytd/1y\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if tree {
				if from != "" {
					return errors.New("--tree shows fixed windows (1d, 3d, 7d, 1m, 3m, ytd, 1y): --from does not apply")
				}
	...
	cmd.Flags().BoolVar(&tree, "tree", false, "envelope-grouped tree: gross/net value and 1d/3d/7d/1m/3m/ytd/1y returns per line")
	return cmd
}
