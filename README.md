...
Say you move 300,000 € of cash into 200,000 € of GTWR and 100,000 € of UETW in your
IBKR account (fake prices):

**Mandatory — record the buys.** This is the whole operation:

```sh
finador asset buy GTWR.DE 4000 @50 --account "IBKR" --group equities/world
...
If you do track cash, model the move out of Nessa as a **pair** (there's no
"transfer" verb), which keeps both balances exact and stays neutral for performance:

```sh
finador cash withdraw "Nessa" 300000     # leaves Nessa
...
- **Never use `cash set` for this.** `set` records an *observed balance*, so dropping
  Nessa from 300k to 0 with `set` would be booked as a −300,000 € loss. `withdraw` is
  the neutral move.
