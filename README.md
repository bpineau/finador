...
- **Exclusions.** Any scope can exclude one or more assets —
  `perf --exclude btc,ddog`, `value equities --exclude aapl`.
...
finador deposit "PEA BforBank" 5000           # external contribution (tax basis, XIRR)
...
finador config set default-account pea-bforbank  # default account for entries
```
